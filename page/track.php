...
$total = mysql_fetch_assoc(mysql_query('SELECT COUNT(*) AS `cnt` FROM `ft_track` WHERE `state`="Y";'));
...
$list = mysql_query('SELECT * FROM `ft_track` WHERE `state`="Y" ORDER BY `tid` DESC LIMIT '.$first_index.', '.$rows.';');
?>

<div class="content_row w604">

<img class="title_img" src="<?=$ft['img_path']?>/title_track.jpg" alt="track title" />
...
<table border="0" cellpadding="0" cellspacing="0" summary="트랙 리스트 테이블" class="list_table mb10">
	...
	<? } ?>
	<? if (!$i) { ?><tr><td colspan="9"><div class="pv20">등록된 트랙이 없습니다.</div></td></tr><? } ?>
	</tbody>
</table>
