...
$(function(){
	...
	$('.timeline_container').mousemove(function(e){
		var pag = e.pageY - 105;
		$('.plus').css({"top": pag + "px", "background" : "url('<?=$ft['wall_path']?>/images/plus.png')", "margin-left" : "1px"});
	}).mouseout(function(){
	...
	$("#update_button").live('click',function(){
		...
		if ($.trim(updateval).length==0) {
			...
			$.ajax({
				type: "POST",
				url: "<?=$ft['wall_path']?>/timeline_message_ajax.php",
				data: dataString,
				cache: false,
	...
	$(".deletebox").live('click',function(){
		...
		if (confirm("Sure you want to delete this update? There is NO undo!")) {
			$.ajax({
				type: "POST",
				url: "<?=$ft['wall_path']?>/delete_message_ajax.php",
				data: dataString,
				cache: false,
				success: function(html){
	...
	$(".stcommentdelete").live('click',function(){
		...
		if (confirm("Sure you want to delete this comment? There is NO undo!")) {
			$.ajax({
				type: "POST",
				url: "<?=$ft['wall_path']?>/delete_comment_ajax.php",
				data: dataString,
				cache: false,
				success: function(html){
	...
	$(".view_comments").live("click",function(){
		...
		$.ajax({
			type: "POST",
			url: "<?=$ft['wall_path']?>/view_ajax.php",
			data: "msg_id="+ ID,
			cache: false,
			success: function(html){
	...
	function more_results() {
		...
		if (ID) {
			$.ajax({
				type: "POST",
				url: "<?=$ft['wall_path']?>/timeline_moreupdates_ajax.php",
				data: "lastid="+ ID +"&profile_uid="+profile_uid,
				cache: false,
				beforeSend: function(){
					$("#more"+ID).html('<img src="<?=$ft['wall_path']?>/icons/ajaxloader.gif" />');
				},
				success: function(html){
					var $boxes = $(html);
	...
	$('.comment_button').live("click",function(){
		...
		if ($.trim(comment).length==0) {
			...
			$.ajax({
				type: "POST",
				url: "<?=$ft['wall_path']?>/comment_ajax.php",
				data: dataString,
				cache: false,
				success: function(html){
					//----
