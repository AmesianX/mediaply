<?
$total = mysql_fetch_assoc(mysql_query('SELECT COUNT(*) AS `cnt` FROM `ft_paid` WHERE `type`="present" AND `price`>0 AND `artist_uid_fk`='.$user['uid'].';'));
...
$list = mysql_query('SELECT * FROM `ft_paid` WHERE `type`="present" AND `price`>0 AND `artist_uid_fk`='.$user['uid'].' ORDER BY `paid_date` DESC, `paid_time` DESC LIMIT '.$first_index.', '.$rows.';');
...
<table border="0" cellpadding="0" cellspacing="0" summary="장미꽃 선물함 리스트 테이블" class="list_table ph20">
	<caption>장미꽃 선물함 리스트 테이블</caption>
	<colgroup>
		<col width="40" />
		<col width="40" />
		<col width="" />
	...
	<tr>
		...
		<td><img src="<?=$ft['img_path']?>/icon_rose.jpg" alt="rose icon" />&nbsp;x<?=$row['price']/10?></td>
